...
用户选择文件
   ...
   └─ .txt  → pathlib.read_text(encoding="utf-8")
    │
    ▼ 返回 {"text": str, "metadata": {filename, page_count, size_bytes, extension, scanned: bool}}
```
...
6. 【重要 — 防幻觉二段判断】在生成回答之前，先快速判断：
   ...
   - 如果检索内容只是碰巧含有关键词、但没有实质回答内容 → 
     直接回复："未在上传文档中找到关于此问题的答案。以下基于通用知识补充：..."
...
class SuperTutorAgent:
    ...
    ingest_document(file_path) → {"ok": bool, "chunk_count"?: int, "reason"?: str}
    ...
    # ── 检索 ──
    search_raw(query, top_k=10) → list[dict]
        宽召回，给规划模块用（不做精排阈值过滤）
...
```
用户选择文件（QFileDialog，筛选 .pdf/.docx/.md/.txt）
    │
    ▼ 前置检查
   ├─ 文件 > 200MB → 拒绝，弹窗提示
...
| ⑦ | LLM 自省判断无实质答案 | System Prompt 第 6 条 | LLM 返回「未在上传文档中找到答案。以下基于通用知识补充：…」 | llm/client.py |
...
**前置条件**（阶段一 `_build_messages()` 中必须落实）：缓存命中的前提是 context 前缀的每个 token 完全一致。因此在拼接 context 时，必须按**确定性顺序**（如按文件名 + chunk_index 排序）拼接，不能依赖检索返回的原始顺序。否则每次 context 顺序随机，缓存永远无法命中。

### 11.3 学习反馈闭环（F-14 / F-15）
...
### 11.5 查询重写（Query Rewriting）

**现状**：用户口语化提问直接检索。

**方案**：检索前用 DeepSeek（~100 tokens）将提问改写为学术关键词 query。例如 "二叉树那章的遍历" → "二叉树 遍历 先序 中序 后序"。BM25 关键词命中率呈指数级上升。
